    ...
    public sealed class VoiceService : IDisposable
    {
        ...
        public void SpeakLap(DataService.LapCompletedArgs args)
        {
            ...
            if (args.LapTime < _bestLap)
            {
                ...
                Enqueue(_normalQueue, new SpeechItem("NewPersonalBest", _settings.GetAlertText("NewPersonalBest", "Nouveau meilleur tour, bravo"), TimeSpan.Zero));
            }
            else
            {
                ...
                if (args.Sector1 > 0 && args.Sector1 < _bestS1)
                {
                    ...
                    Enqueue(_normalQueue, new SpeechItem("SectorBeat_S1", _settings.GetAlertText("SectorBeat_S1", "Secteur un, meilleur temps"), TimeSpan.Zero));
                }
                if (args.Sector2 > 0 && args.Sector2 < _bestS2)
                {
                    _bestS2 = args.Sector2;
                    Enqueue(_normalQueue, new SpeechItem("SectorBeat_S2", _settings.GetAlertText("SectorBeat_S2", "Secteur deux, meilleur temps"), TimeSpan.Zero));
                }
                if (args.Sector3 > 0 && args.Sector3 < _bestS3)
                {
                    _bestS3 = args.Sector3;
                    Enqueue(_normalQueue, new SpeechItem("SectorBeat_S3", _settings.GetAlertText("SectorBeat_S3", "Secteur trois, meilleur temps"), TimeSpan.Zero));
                }
            }
        }
        ...
        private void CheckFlags(DataService ds)
        {
            ...
            if (flag == 3 && _prevFlag != 3)
                Enqueue(_urgentQueue, new SpeechItem("RedFlag", _settings.GetAlertText("RedFlag", "Drapeau rouge, arrêt immédiat"), TimeSpan.Zero));

            // Drapeau vert
            if (flag == 1 && _prevFlag != 1)
                Enqueue(_normalQueue, new SpeechItem("GreenFlag", _settings.GetAlertText("GreenFlag", "Drapeau vert, go"), TimeSpan.Zero));

            // Drapeau à damier
            if (flag == 8 && !_checkeredAnnounced)
            {
                _checkeredAnnounced = true;
                Enqueue(_normalQueue, new SpeechItem("CheckeredFlag", _settings.GetAlertText("CheckeredFlag", "Drapeau à damiers, belle course"), TimeSpan.Zero));
            }
            ...
            if ((flag == 2) && ys != _prevYellowState)
            {
                ...
                {
                    case 1:
                        Enqueue(_urgentQueue, new SpeechItem("YellowFlag", _settings.GetAlertText("YellowFlag", "Drapeau jaune, prudence"), TimeSpan.FromSeconds(_settings.CooldownYellowFlagSeconds)));
                        break;
                    case 2:
                        Enqueue(_urgentQueue, new SpeechItem("YellowFlagPitClosed", _settings.GetAlertText("YellowFlagPitClosed", "Jaune, pit fermé"), TimeSpan.FromSeconds(_settings.CooldownYellowFlagSeconds)));
                        break;
                    case 4:
                        Enqueue(_normalQueue, new SpeechItem("YellowFlagResume", _settings.GetAlertText("YellowFlagResume", "Reprise, drapeau vert"), TimeSpan.Zero));
                        break;
                }
            }
        ...
        private void CheckFuel(DataService ds)
        {
            ...
            if (fd.WindowState == PitWindowState.WindowOpen && !_fuelWindowAnnounced)
            {
                ...
                Enqueue(_normalQueue, new SpeechItem("FuelWindowOpen", _settings.GetAlertText("FuelWindowOpen", "Fenêtre de pit ouverte"), TimeSpan.Zero));
            }

            if (fd.RealAutonomy < 3.0 && !_fuelLowAnnounced)
            {
                _fuelLowAnnounced = true;
                Enqueue(_urgentQueue, new SpeechItem("FuelLowLaps", _settings.GetAlertText("FuelLowLaps", "Moins de trois tours de carburant"), TimeSpan.Zero));
            }

            if (fd.WindowState == PitWindowState.Critical || fd.RealAutonomy < 1.5)
                Enqueue(_urgentQueue, new SpeechItem("FuelCritical", _settings.GetAlertText("FuelCritical", "Carburant critique"), TimeSpan.FromSeconds(_settings.CooldownFuelCriticalSeconds)));
        }
        ...
        private void CheckGap(DataService ds)
        {
            ...
            if (behind != null && gapBehind > 0 && gapBehind < thresh)
                Enqueue(_normalQueue, new SpeechItem("GapCloseBehind", _settings.GetAlertText("GapCloseBehind", "Attention derrière"), TimeSpan.FromSeconds(_settings.CooldownGapSeconds)));

            if (ahead != null && _prevGapAhead > 0 && _prevGapAhead < thresh * 2 && gapAhead > thresh * 3)
                Enqueue(_normalQueue, new SpeechItem("GapLostAhead", _settings.GetAlertText("GapLostAhead", "Tu décroches de la voiture devant"), TimeSpan.FromSeconds(_settings.CooldownGapSeconds)));
        ...
        private void CheckPosition(DataService ds)
        {
            ...
            if (_prevClassPosition > 0 && classPos != _prevClassPosition)
            {
                if (classPos < _prevClassPosition)
                    Enqueue(_normalQueue, new SpeechItem("PositionGained", _settings.GetAlertText("PositionGained", "Position gagnée"), TimeSpan.FromSeconds(_settings.CooldownPositionSeconds)));
                else
                    Enqueue(_normalQueue, new SpeechItem("PositionLost", _settings.GetAlertText("PositionLost", "Position perdue"), TimeSpan.FromSeconds(_settings.CooldownPositionSeconds)));
            }
            ...
            else
            {
                ...
                if (_spotterState != 0)
                {
                    ...
                    if (_spotterClearCount >= 4) // ~1.3 s à 3 Hz
                    {
                        _spotterState      = 0;
                        _spotterClearCount = 0;
                        Enqueue(_normalQueue, new SpeechItem("SpotterClear", _settings.GetAlertText("SpotterClear", "Dégagé"),
                            TimeSpan.FromSeconds(_settings.CooldownSpotterSeconds)));
                    }
                }
            }
