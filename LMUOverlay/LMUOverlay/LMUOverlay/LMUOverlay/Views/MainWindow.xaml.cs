    ...
    public partial class MainWindow : Window
    {
        ...
        private void SelectOverlay(string key)
        {
            ...
            if (key == "InputGraph")
            {
                ...
                // History duration
                AddSlider("Historique (secondes)", _config.InputConfig.HistorySeconds, 1, 15,
                    v => _config.InputConfig.HistorySeconds = (int)Math.Round(v), "F0");
        ...
        private void ShowTab(string tab)
        {
            ...
            TabThemes.BorderBrush       = tab == "themes"      ? _tabUnderlineActive : _tabUnderlineInactive;
            TabThemes.Foreground        = tab == "themes"      ? _tabFgActive        : _tabFgInactive;
        }
        ...
        private bool _themesInitialized;
        private void OnTabThemes(object s, RoutedEventArgs e)
        {
            ShowTab("themes");
            if (!_themesInitialized)
            {
                _themesInitialized = true;
                ThemesPanel.Initialize(_config, _configService);
            }
        }
