        ...
        private void InstallHooks()
        {
            try
            {
                ...
                try
                {
                    ...
                    var mod = proc.MainModule; // can be null on some systems with security software
                    if (mod != null)
                        hMod = GetModuleHandle(mod.ModuleName);
                }
                catch (Exception ex)
                {
                    // MainModule access denied on some systems — fallback to null module handle
                    // Low-level hooks still work with IntPtr.Zero on most Windows versions
                    Debug.WriteLine($"[HotkeyService] MainModule unavailable: {ex.Message}");
                }
            ...
            {
                Debug.WriteLine($"[HotkeyService] InstallHooks failed: {ex.Message}");
            }
        }
