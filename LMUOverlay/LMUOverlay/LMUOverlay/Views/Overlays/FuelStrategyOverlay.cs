...
namespace LMUOverlay.Views.Overlays
{
    public class FuelStrategyOverlay : BaseOverlayWindow
    {
        ...
        public FuelStrategyOverlay(DataService ds, OverlaySettings s) : base(ds, s)
        {
            ...
            _fuelBarBg = MakeBarBg(Color.FromRgb((byte)(tm0.StateGood.R / 2), (byte)(tm0.StateGood.G / 5), (byte)(tm0.StateGood.B / 6)));
            ...
            // ── ENERGY BAR ───────────────────────────────────────────────────
            _energyBarBg = MakeBarBg(Color.FromRgb((byte)(tm0.ClassLmp2.R / 6), (byte)(tm0.ClassLmp2.G / 6), (byte)(tm0.ClassLmp2.B / 7)));
            ...
            _fuelToAdd = new TextBlock { FontSize = 20, FontWeight = FontWeights.Bold, FontFamily = OverlayHelper.FontConsolas, Foreground = BrushCache.Get(ThemeManager.Current.StateWarn), HorizontalAlignment = HorizontalAlignment.Center };
            ...
            // ── SEPARATOR ────────────────────────────────────────────────────
            main.Children.Add(new Border { Height = 1, Background = BrushCache.Get(ThemeManager.Current.Border), Margin = new Thickness(0, 2, 0, 2) });
        ...
        public override void UpdateData()
        {
            ...
            if (fPct <= 0.05)
            {
                ...
                _fuelFill.Background = BrushCache.Get(on ? tm.StateDanger : Color.FromRgb((byte)(tm.StateDanger.R / 2), 10, 10));
                _fuelBarBg.BorderBrush = on ? BrushCache.Get(tm.StateDanger) : null;
            ...
            else
            {
                ...
                _fuelFill.Background = fPct > 0.50
                    ? BrushCache.Get(tm.StateGood)
                    : fPct > 0.25
                        ? BrushCache.Get(tm.StateWarn)
                        : BrushCache.Get(tm.StateDanger);
            }
            ...
            if (showVE)
            {
                ...
                if (ePct <= 0.05)
                {
                    _flashCounter++;
                    bool on = (_flashCounter / 4) % 2 == 0;
                    _energyFill.Background = BrushCache.Get(on ? tm.StateDanger : Color.FromRgb((byte)(tm.StateDanger.R / 2), 10, 10));
                    _energyBarBg.BorderBrush = on ? BrushCache.Get(tm.StateDanger) : null;
            ...
            if (showVE)
            {
                ...
                {
                    _energyToAdd.Text = "-- %";
                    _energyToAdd.Foreground = BrushCache.Get(tm.TextMuted);
                }
            }
            ...
            if (f.RealAutonomy > 0)
            {
                ...
                _autonomyText.Foreground = BrushCache.Get(
                    f.RealAutonomy < 3 ? tm.StateDanger :
                    f.RealAutonomy < 5 ? tm.StateWarn :
                    tm.StateGood);
            }
            else
            {
                _autonomyText.Text = "--";
                _autonomyText.Foreground = BrushCache.Get(tm.TextMuted);
            }
            ...
            {
                LimitingFactor.Fuel   => tm.AccentSecondary,
                LimitingFactor.Energy => tm.StateBestLap,
                _                     => tm.TextMuted
            });
            ...
            if (showPitDist)
            {
                ...
                if (inPits && pitState == 3)
                {
                    ...
                    _pitDistValue.Foreground = BrushCache.Get(tm.StateGood);
                }
                ...
                else if (pitState == 4)
                {
                    ...
                    _pitDistValue.Foreground = BrushCache.Get(tm.ClassLmp2);
                }
                else
                {
                    ...
                    _pitDistValue.Foreground = BrushCache.Get(
                        distToPit <= 150  ? tm.StateDanger :
                        distToPit <= 400  ? tm.StateWarn :
                        pitState   >= 1   ? tm.ClassLmp2 :
                                            tm.TextSecondary);
                }
            }
            ...
            _stopsLeft.Foreground = BrushCache.Get(f.StopsRequired == 0 ? tm.StateGood : tm.StateWarn);
            _compound.Text = !string.IsNullOrEmpty(p.TireCompound) ? p.TireCompound : "--";
            _tireWear.Text = $"{p.TireWearAvg * 100:F0}%";
            _tireWear.Foreground = BrushCache.Get(p.TireWearAvg > 0.5 ? tm.StateGood : p.TireWearAvg > 0.25 ? tm.StateWarn : tm.StateDanger);
        ...
        static void C(Grid g, string l, out TextBlock v, int r, int c)
        {
            ...
            s.Children.Add(L(l));
            v = new TextBlock { FontSize = 12, FontWeight = FontWeights.Bold, FontFamily = OverlayHelper.FontConsolas, Foreground = BrushCache.Get(ThemeManager.Current.TextPrimary), HorizontalAlignment = HorizontalAlignment.Center };
            s.Children.Add(v);
