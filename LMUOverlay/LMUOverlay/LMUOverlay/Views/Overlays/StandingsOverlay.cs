...
namespace LMUOverlay.Views.Overlays
{
    public class StandingsOverlay : BaseOverlayWindow
    {
        ...
        private UIElement Row(VehicleData v, int classPos, Color cc, int idx)
        {
            ...
            Color bg = isP ? Color.FromArgb(45, tm.AccentPrimary.R, tm.AccentPrimary.G, tm.AccentPrimary.B) : idx % 2 == 0 ? Color.FromArgb(10, tm.TextPrimary.R, tm.TextPrimary.G, tm.TextPrimary.B) : Colors.Transparent;

            var row = new Border { Background = BrushCache.Get(bg), Padding = new Thickness(1, 1, 1, 1), MinHeight = 17 };
            if (isP) { row.BorderBrush = BrushCache.Get(tm.AccentPrimary); row.BorderThickness = new Thickness(2, 0, 0, 0); }
            ...
            // Driver
            if (_colCfg.Driver)
                P(g, c++, new TextBlock { Text = OverlayHelper.FormatName(v.DriverName), FontSize = F, FontWeight = isP ? FontWeights.Bold : FontWeights.Normal, FontFamily = _consolas, Foreground = isP ? BrushCache.Get(tm.AccentPrimary) : BrushCache.Get(tm.TextPrimary), VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(3, 0, 0, 0), TextTrimming = TextTrimming.CharacterEllipsis });
            ...
            if (_colCfg.BestLap)
            {
                ...
                P(g, c++, T(bl, F, BrushCache.Get(tm.StateGood), HorizontalAlignment.Right));
            }
            ...
            if (_colCfg.LastLap)
            {
                ...
                Color lc = v.LastLapTime > 0 && Math.Abs(v.LastLapTime - v.BestLapTime) < 0.001
                    ? tm.StateGood : v.LastLapTime > 0
                    ? tm.TextPrimary : tm.TextMuted;
            ...
            if (_colCfg.Delta)
            {
                ...
                Color dc = v.LastLapDelta < 0 ? tm.StateGood : v.LastLapDelta > 0 ? tm.StateWarn : tm.TextMuted;
            ...
            if (_colCfg.Damage)
            {
                Color dmgC = v.DamagePercent > 50 ? tm.StateDanger : v.DamagePercent > 20 ? tm.StateWarn : tm.TextMuted;
            ...
            if (_colCfg.Indicator)
            {
                ...
                {
                    1 => tm.ClassLmp2,                  // blue flag
                    2 => tm.StateWarn,                  // yellow flag
                    3 => Color.FromRgb(30, 30, 30),    // black flag (dark)
                    4 => tm.TextPrimary,                // checkered
                    5 => tm.StateDanger,                // DQ
                    _ => Colors.Transparent
                };
                if (v.InPits) ic = tm.AccentSecondary; // pit lane override
        ...
        private static Border SD(SectorStatus s, bool cur)
        {
            ...
            Color bg = s switch
            {
                SectorStatus.SessionBest  => tm.StateBestLap,
                SectorStatus.PersonalBest => tm.StateGood,
                SectorStatus.Slower       => tm.StateWarn,
                _                         => Color.FromArgb(30, 90, 120, 120)
            };
        ...
        private static Color ClassCol(string c)
        {
            var tm = ThemeManager.Current;
            return c switch
            {
                "HYPERCAR" => tm.ClassHypercar,
                "LMP2"     => tm.ClassLmp2,
                "LMP3"     => Color.FromRgb(140, 140, 0),
                "LMGTE"    => tm.ClassLmgt,
                "GT3"      => tm.ClassGt3,
                "GT4"      => Color.FromRgb(100, 60, 140),
                _          => tm.TextMuted
            };
        }
